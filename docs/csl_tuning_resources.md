...
> **関連文書 — 本リポジトリの解析（いずれもパラメータ定義名とアドレスのレベル、データログ戦略つき）:**
>
> - [軽量フライホイール装着車のドライバビリティ・チューニング](lightweight_flywheel_tuning.md)
>   — 回転慣性を下げたときに崩れる制御ループを 7 領域にわたって洗い出した解析。
> - [CSL コンバート車のアイドル過濃と低回転ガクつき](csl_conversion_idle_and_tipin.md)
>   — スノーケル・フラップ非装着での症状を、Alpha-N 充填量モデル (`kf_rf_soll`) と
>   MAP 自己補正の凍結という観点から追った解析。
> - [クイックステアリングラック装着時の DME 調整](quick_steering_rack_tuning.md)
>   — 舵角がアイドルの点火権限を握っている経路と、ラック比変更で失われるもの。

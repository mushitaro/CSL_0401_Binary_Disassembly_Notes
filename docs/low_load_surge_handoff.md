...
| 位相余裕 39.5° → 48.8° | **未検証。** しかも**実車に入れたのは 4 行版**（46.6°）で、行 15 は未適用 |
| TUNER 実装 | **未着手** ← ここから |
...
- **アンカーは `ml` = 30 行**。据え置き、そこから `k = RF₃₀ / 30` を得る
  （**引数にする** — 理論上どこでもよく、実測でも差は 2° 以内。§9.9-8c）
- 各行 `RF_target = k · ml` → `kf_rf_soll⁻¹`（二分法）→ `q` → `A` → `KL_AQ_ABS_LLS⁻¹`（線形逆補間）
- `clamp(D, K_LLS_TV_MIN, K_LLS_TV_MAX)` = 14 – 97 %
- **書き換える行は 15 / 20 / 25 / 40 / 50。** 規則は「実測点が補間で読む区間の端点」で、
  それは 15/20/25/30/40/50。**30 はアンカーなので定義上不変**、行 11 と 60 以上は読まれない
- **アンカーは行 30。積極的な理由は無い**（§9.9-8c）。理論上どこでもよく（目標 `RF = k·ML` の
  弾性は `k` に依らず 1、アンカーは列の高さだけを決め、高さは `FR_REGLER` が吸収する）、
  実測でも差は 2° 以内。**純関数はアンカー行を引数に取ること**
...
| 2 | 逆引き表 | §9.8 の表、**5 行 × 4 列を全セル**（800/50 と 1400/50 は変化なしが正解） | §9.8 |
| 3 | 位相余裕（320 点） | 平均 **39.5° → 48.8°**、PM<30° が **25.3 % → 1.9 %** | §9.9-8 |
| 4 | 区間別 | 15–20 区間が **47.0° → 56.3°**（4 行版だと 39.7° に悪化する） | §9.9-8a |
| 5 | アンカー非依存 | アンカーを 20/25/30/40 に変えても平均 **48.4–49.0°** に収まる | §9.9-8c |

> **3 の母集団は 362 行ではなく `Road Speed` > 0 の 320 行**、`RF` は `kf_rf_soll` の
> 再評価ではなく**ログの `relative Fuellung`**。これを外すと現行側が 39.5° にならない。
>
> **4 が落ちたら行 15 を書いていない。** これが実装で一番落ちやすい。

**併せて固定すべき不変量**（回帰で落ちやすい所）：

- アンカー行（`ml` = 30）が**ビット単位で不変**であること
- 行 11 / 30 / 60 以上に**一切書かない**こと（30 はアンカー、他は実測点が読まない）
...
| 事項 | 状態 |
|---|---|
| 39.5° → 48.8° が実車で起きたか | **未測定。** 2 本目のログ（同じ左旋回）が要る |
| 行 15 の新値（31.8 / 33.7 / 33.9 / 34.3） | **未適用。** 実車に入っているのは 4 行版まで |

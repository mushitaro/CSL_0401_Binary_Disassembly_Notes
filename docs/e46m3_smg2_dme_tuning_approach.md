...
**(3) ただし収穫はあります。** CSL リファレンスとの **47 パラメータの差分**があり、
...
| **較正状態** | パラメータが触られているか | **触られている。47 項目**（§7） |
...
> **注意**: `K_VERS_DATEN` は書き換え可能なただのパラメータで、80 → 81 はチューナーが
> 1 バイト上げるだけの操作です。**「両車が同じ配布ファイルを土台にしている」証拠にはなりません。**
...
実際 UPLOAD の較正は 47 項目が動いており（§7）、その手法は §7.5a に整理してあります。
...
共通しているのは「0401 であること」と、§8 で見る **12 個の較正値**だけです。
...
### 8.1 値まで完全に同じ 12 項目 = 「共通のレシピ」

上の「値まで同一 13」のうち **`K_FGR_CONFIG` は数に入れてはいけません**。UPLOAD・CSLcar・TERRA
がそろって `3` で、`99` を持っているのは **CPv1 だけ**です。両車が同じ編集をしたのではなく、
物差しに使った CPv1 のほうがここで動いているだけ。**実質は 12 項目**です。

`DTC_7C_CSL_FLAP_POT` / `DTC_B2_CAT_PROTECT_BANK1` / `DTC_B3_CAT_PROTECT_BANK2` /
`KL_V_MAX_GANG` / `K_AVAN1_OFFSET` / `K_EVAN1_OFFSET` / `K_FDYN_CONTROL` /
`K_N_MAX_VFEHLER` / `K_SLS_UB_MAX` / `K_SLS_UB_MIN` / `K_VERS_DATEN_M` / `K_VERS_DATEN_S`

この 12 項目は **TERRA とも CPv1 とも違う値で、両車が一致**しています。物差しを CPv1 から
TERRA に替えても同じ 12 項目が残る、という意味です。

**では「同じ配布ファイルを土台にしている」のか — それは言えません。** 素性は §1.3a のとおり
別系譜で、`K_VERS_DATEN` の 81 も 1 バイト書けば揃う値です。土台が違うのに 12 個の値が
...
いずれにせよ結論は変わりません — **この 12 項目は個々のチューナーの判断ではないので、

...
**(4) UPLOAD は Terra 系でも Community Patch 系でもなく、素の CSL 系譜です。**
系譜フィンガープリント 2 つ（`K_FR_T_ADAPT` = `00 96`、排ガス検査端末レコードがどちらの
位置にも無い）が独立に一致します。しかも HW は `21132500` — CSL 純正ブートローダなので、
`00 96` はそこでは正常値です。**「CSL の DME に CSL のソフト」という矛盾のない組み合わせ**で、
お手元の車（HW `21132300` + Community Patch v1）とは**別系譜**です。詳細は §1.3a。
...
|---|---|---|---|---|---|
| **UPLOAD** | **21132500** | **`00 96`** | `FF FF FF` | `FF FF FF` | **素の CSL** |
...
**UPLOAD は Terra 系でも Community Patch 系でもなく、素の CSL です。**
...
- **`K_FR_T_ADAPT`。UPLOAD の `00 96`（1.50 s）が素の CSL 出荷値**で、`01 96`（4.06 s）を
...
| `K_FR_T_ADAPT` | constant | 0xE002 | 0x0E002 | master | 4.06 → 1.5 | sec | 充填レギュレータの適応時定数 | **UPLOAD 側は無編集**（1.5 = 素の CSL 出荷値、§7.0 C 群） | — | measured |

> **`K_FR_T_ADAPT` の行だけは向きが逆です。** `0x0096` = 150 → 1.50 s が素の CSL の出荷値で、
...
**2 台は別々のハードウェア・別々のプログラム系譜です**（§1.3a）。UPLOAD は HW 21132500 の
素の CSL、CSLcar は HW 21132300 に CSL 0401 を載せた Community Patch v1 系。
...
素の CSL 値 1.5 s を保っているのに対し、お手元の車が Community Patch v1 の 2.56 s を
